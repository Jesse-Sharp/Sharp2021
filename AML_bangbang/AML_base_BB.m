...
omega = 0.4; %Portion of previous iteration's control maintained when updating control
...
Err = inf; %Initialise the error
 ...
 while Err > tol
     ...
     [y,Lambda,Uupdate] = FBSM_BB(y0,U,parameters); %Forward Backward sweep (1 iteration)
     Fevals = Fevals + 1;
     ...
     %%To view the solution as it converges, uncomment this block (line 54 to line 68)
...
figure
[y,~,~] = FBSM_BB(y,U,parameters);  %Compute state ODE based on most recent control
box on
line1 = plot(t_y,y(:,1),'LineWidth',2);
hold on
line2 = plot(t_y,y(:,2),'LineWidth',2);
