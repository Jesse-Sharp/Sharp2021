...
while j > 1 
    i = i+1;
    j = N-i;
    k1 = Costate_BB(Lambda(j+1,:),y(j+1,:),parameters);
    k2 = Costate_BB(Lambda(j+1,:)-dt*k1/2,0.5*(y(j,:)+y(j+1,:)),parameters);
    k3 = Costate_BB(Lambda(j+1,:)-dt*k2/2,0.5*(y(j,:)+y(j+1,:)),parameters);
    k4 = Costate_BB(Lambda(j+1,:)-dt*k3,y(j,:),parameters);
    Lambda(j,:) = Lambda(j+1,:) - (dt/6)*(k1+2*k2+2*k3+k4);
end
Uupdate= Control_BB(Lambda,parameters); %Calculated updated control
