...
parameters.Uupper = 2; %Upper bound on control
parameters.Ulower = 0; %Lower bound on control
...
while Err > tol
    ...
    [~,~,Uupdate] = FBSM_BB(y0,U,parameters);
    ...
    %%To view the solution as it converges, uncomment this block (line 45 to line 59)
    %%to produce interim figures
%         figure(1)
%         box on
%         [y,~,~] = FBSM_BB(y0,U,parameters); %Compute state ODE based on most recent control
