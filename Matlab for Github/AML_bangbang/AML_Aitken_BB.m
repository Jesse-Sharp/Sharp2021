...
%  
function [Control,Fevals] = AML_Aitken_BB(tol,MaxFevals,m,omega)
...
while Err > tol
    ...
    X(:,1) = X(:,end);
    [~,~,Xn] = FBSM_BB(y0,X(:,1),parameters); 
    X(:,2) = omega*X(:,1)+(1-omega)*Xn; %Apply relaxation factor to aid convergen
    
    Err = norm(Xhatprev-Xhat);
    
    %%To view the solution as it converges, uncomment this block (line 96 to line 111)
    %%to produce interim figures
%     figure(1)
%     box on
%     [y,~,~] = FBSM_BB(y0,Xhat,parameters); %Compute state ODE based on most recent control
