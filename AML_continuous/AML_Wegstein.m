...
%  
function [Control,Fevals] = AML_Wegstein(tol,MaxFevals,nth,bounding,lower,upper,omega)
...
while Err > tol
    ...
    if iterations == 1 %Compute Wegstein's q on first iteration
        ...
        %to accelerate convergence
        Xn = q.*Xn + (1-q).*Fn;
        [~,~,Fn] = FBSM(y0,Xn,parameters); %Solve system with updated control to obtain next control value
        Fevals = Fevals+1;
        Err = norm(Fn-Xn);
    end
    if Fevals > MaxFevals
    ...
    %%To view the solution as it converges, uncomment this block (line 117 to line 132)
    %%to produce interim figures
%     figure(1)
%     box on
%     [y,~,~] = FBSM(y0,Fn,parameters); %Compute state ODE based on most recent control
