...
%  
function [Control,Fevals] = AML_Anderson(tol,MaxFevals,M,Droptol,omega)
...
while  Err > tol
    ...
    if m < M %don't need to replace old matrix entries
        ...
        [~,~,Xn] = FBSM(y0,X(:,end),parameters); %Solve system with updated control
    ...
    %%To view the solution as it converges, uncomment this block (line 105 to line 120)
    %%to produce interim figures
%     figure(1)
%     box on
%     [y,~,~] = FBSM(y0,X(:,end),parameters); %Compute state ODE based on most recent control
